...
if ($compiledArtifactFolder -ne "none"){
  ...
  write-output "of `"$BITBUCKET_CLONE_DIR\$compiledArtifactFolder`"" >> script.scan
  ...
  write-host "Config file created for compiled folder ($BITBUCKET_CLONE_DIR\$compiledArtifactFolder)."
}
else{
  write-output "login_file $aseHostname `"$aseToken`" -acceptssl" > script.scan
  write-output "RUNAS AUTO" >> script.scan
  write-output "of `"$BITBUCKET_CLONE_DIR`"" >> script.scan
